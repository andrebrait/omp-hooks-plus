...
For explicit file inputs, resource inventory is intentionally limited to the selected `--include` paths. Exclusion reporting covers that selected subtree; omitted-resource reporting does not enumerate unselected project files. File input is not permission to inspect the surrounding project tree. Whole-plugin omission reporting applies only to plugin-directory input.
...
The source root's canonical path and directory identity are captured during source loading and checked again through resource collection and publication. JSON and scoped frontmatter share a contained, no-follow descriptor reader: it verifies the opened file's device/inode and path identity before parsing, then reads from that descriptor. Replacing a declaration path or source root fails closed rather than importing a replacement tree.
