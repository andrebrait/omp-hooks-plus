...
Reports include structured activation requirements: deployment policy (explicit standalone OMP activation or existing automatic bridge controls), required host capabilities/controls, source scope and disable directives, ownership protocol requirements, and known conflicting original entrypoints. These describe requirements, not a cached authorization decision; runtime activation rechecks the actual host state. They cannot grant trust, enable a disabled source, or authorize coexistence with an old non-cooperative bridge.
...
2. **Query:** after the host's factory-binding barrier, the first applicable dispatch broadcasts `kind: "query"` with the selected participant roster and its digest. Derive `requestId` as the SHA-256 of the UTF-8 JSON array `["omp-hook-converter:ownership", 1, sessionKey, generation, rosterDigest]`, using the host snapshot's exact values and canonical roster ordering. Independently initiated queries therefore use one request ID for that session/generation/roster and coalesce into one local pending decision per bundle. A duplicate query re-sends the current state/ack as appropriate but never resets the deadline or starts another ownership decision. A roster inconsistent with the host snapshot is a conflict, not a second election. All participants apply the same selection rules and acknowledge only the seal derived from that shared request.
