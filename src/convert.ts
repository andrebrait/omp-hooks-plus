...
export type ConvertOptions = {
  ...
  approximate?: boolean;
  /** Name shown as the reminders' source; defaults to the plugin name, else omp-hooks-plus. */
  sourceName?: string;
};
...
function generatedEntrypoint(source: ConversionSource, resourceHash: string, activation: "enabled" | "project-trusted", approximate: boolean, sourceName: string): string {
...
export async function convertHooks(input: string, options: ConvertOptions = {}): Promise<ConvertResult> {
  ...
  if (options.sourceName !== undefined && !/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(options.sourceName)) {
    throw new Error("--source-name must start with a letter or digit and contain only letters, digits, '.', '_' or '-'");
  }
...
if (import.meta.main) {
  try {
    ...
    if (values.help) {
      console.log("Usage: bun run convert <plugin-directory|hooks.json|settings.json> --out <new-directory> [--source-root <directory>] [--include <relative-resource>] [--activation <enabled|project-trusted>] [--skip-unsupported] [--approximate] [--source-name <name>] [--dry-run] [--json]");
    } else {
