...
function generatedEntrypoint(source: ConversionSource, resourceHash: string): string {
...
export async function convertHooks(input: string, options: ConvertOptions = {}): Promise<ConvertResult> {
  ...
  try {
    ...
    if (adapter) {
      ...
      writeFileSync(temporary, generatedEntrypoint(source, resourceHash.digest("hex")), { flag: "wx" });
