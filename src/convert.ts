...
function generatedEntrypoint(source: ConversionSource, resourceHash: string, activation: "enabled" | "project-trusted"): string {
...
export async function convertHooks(input: string, options: ConvertOptions = {}): Promise<ConvertResult> {
  ...
  try {
    ...
    if (adapter) {
      ...
      writeFileSync(temporary, generatedEntrypoint(source, resourceHash.digest("hex"), activation), { flag: "wx" });
...
if (import.meta.main) {
  try {
    ...
    if (values.help) {
      console.log("Usage: bun run convert <plugin-directory|hooks.json|settings.json> --out <new-directory> [--source-root <directory>] [--include <relative-resource>] [--activation <enabled|project-trusted>] [--dry-run] [--json]");
    } else {
